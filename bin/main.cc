...
main(int argc, char *argv[])
{
	...
	svr4 = false;
	posix = false;
	if(!strcmp(argv_zero_string, "/usr/local/bin/sunpromake")) {
		svr4 = false;
		posix = true;
	} else {
 ...
 */
	...
	dmake_name2 = GETNAME(wcs_buffer, FIND_LENGTH);
	prop2 = get_prop(dmake_name2->prop, macro_prop);
	// If we're invoked as 'sunpromake' run serially, regardless of DMAKE_MODE
	// If we're invoked as 'sunpromake' but passed -j, run parallel
	// If we're invoked as 'dmake', without DMAKE_MODE, default parallel
	// If we're invoked as 'dmake' and DMAKE_MODE is set, honour it.
	if ((strcmp(b, "sunpromake") == 0) &&
